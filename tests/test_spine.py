...
def test_read_spine_path_override_reads_a_different_file(tmp_path: Path) -> None:
    ...
    instead of the default live spine.jsonl — used by a reader to
    ...
    cold.write_text('{"source":"drain","event":"transcript-synced"}\n')
    recs = read_spine(tmp_path, source="drain", path=cold)
    assert [r["event"] for r in recs] == ["transcript-synced"]
    # Default behaviour (no path=) is unchanged — still the live spine.
    assert read_spine(tmp_path, source="drain") == []
