...
def test_merge_gate_uses_files_modified_with_legacy_fallback(lore_root):
    """The merge gate's Jaccard signal switches to ``files_modified``;
    legacy notes carrying only ``files_touched`` still match via the
    fallback so historical merging keeps working."""
    from lore_core.identity import session_note_dir
    from lore_core.io import atomic_write_text
    from lore_core.session_writer import _find_todays_open_note

    wiki_root = lore_root / "wiki" / "private"
    sessions_base = session_note_dir(wiki_root, "")
    month_dir = sessions_base / "2026" / "05"
    month_dir.mkdir(parents=True)

    # Candidate 1: legacy shape (only ``files_touched``).
    legacy_note = month_dir / "01-1000-legacy.md"
    atomic_write_text(legacy_note, (
        "---\n"
        "type: session\n"
        "scope: proj:feature\n"
    ...
    # Candidate 2: new shape (only ``files_modified``).
    new_note = month_dir / "01-1100-new.md"
    atomic_write_text(new_note, (
        "---\n"
        "type: session\n"
        "scope: proj:feature\n"
        "files_modified: ['/repo/auth.py']\n"
        "---\n\n# new\n"
    ))

    scope = _scope()
    work_date = datetime(2026, 5, 1, 14, 0, tzinfo=UTC).date()

    # Caller with new-shape signal lands on candidate 2 (highest Jaccard
    # via the new field). Candidate 1's ``files_touched`` is still a
    # valid match if the new field is missing — see the next assertion.
    hit_new = _find_todays_open_note(
        sessions_base, scope=scope, work_date=work_date,
        new_files_modified=["/repo/auth.py"],
        new_transcript_id=None,
    )
    assert hit_new in (legacy_note, new_note)

    # Caller with no ``files_modified`` should still match via fallback.
    hit_legacy = _find_todays_open_note(
        sessions_base, scope=scope, work_date=work_date,
        new_files_touched=["/repo/auth.py"],
        new_transcript_id=None,
    )
    assert hit_legacy in (legacy_note, new_note)
