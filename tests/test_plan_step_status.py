...
def test_set_step_done(plan_with_steps: tuple[Path, str]) -> None:
    ...
    update = set_step(wiki_root=wiki, slug=slug, step_id="step-2", status=StepStatus.DONE)
    assert update.previous is None
    assert update.current == "done"
    fm = parse_frontmatter(plan_path(wiki, slug).read_text())
    assert fm["step_status"] == {"step-2": "done"}
    assert "step_status_updated" in fm


def test_set_step_in_progress_then_done(plan_with_steps: tuple[Path, str]) -> None:
    wiki, slug = plan_with_steps
    set_step(wiki_root=wiki, slug=slug, step_id="step-1", status="in_progress")
    update = set_step(wiki_root=wiki, slug=slug, step_id="step-1", status="done")
    assert update.previous == "in_progress"
    assert update.current == "done"
    fm = parse_frontmatter(plan_path(wiki, slug).read_text())
    assert fm["step_status"]["step-1"] == "done"


def test_set_step_blocked(plan_with_steps: tuple[Path, str]) -> None:
    wiki, slug = plan_with_steps
    set_step(wiki_root=wiki, slug=slug, step_id="step-2", status=StepStatus.BLOCKED)
    fm = parse_frontmatter(plan_path(wiki, slug).read_text())
    assert fm["step_status"]["step-2"] == "blocked"
...
def test_set_step_clear_via_none(plan_with_steps: tuple[Path, str]) -> None:
    ...
    wiki, slug = plan_with_steps
    set_step(wiki_root=wiki, slug=slug, step_id="step-1", status="done")
    update = set_step(wiki_root=wiki, slug=slug, step_id="step-1", status=None)
    assert update.previous == "done"
    assert update.current is None
    fm = parse_frontmatter(plan_path(wiki, slug).read_text())
    assert "step-1" not in (fm.get("step_status") or {})
...
def test_set_step_unknown_step_id_raises(plan_with_steps: tuple[Path, str]) -> None:
    ...
    with pytest.raises(ValueError, match="not in plan"):
        set_step(wiki_root=wiki, slug=slug, step_id="step-99", status="done")
...
def test_set_step_invalid_status_string_raises(
    ...
    with pytest.raises(ValueError, match="unknown step status"):
        set_step(wiki_root=wiki, slug=slug, step_id="step-1", status="finished")
...
def test_set_step_unknown_plan_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(FileNotFoundError):
        set_step(wiki_root=wiki, slug="nonexistent", step_id="step-1", status="done")
...
def test_step_status_updated_bumps_on_change(
    ...
    set_step(wiki_root=wiki, slug=slug, step_id="step-1", status="in_progress", now=t1)
    set_step(wiki_root=wiki, slug=slug, step_id="step-1", status="done", now=t2)
    fm = parse_frontmatter(plan_path(wiki, slug).read_text())
...
def test_advance_picks_first_in_progress_when_present(
    ...
    wiki, slug = plan_with_steps
    set_step(wiki_root=wiki, slug=slug, step_id="step-2", status="in_progress")
    update = advance(wiki_root=wiki, slug=slug)
    assert update is not None
    assert update.step_id == "step-2"
    assert update.current == "done"
...
def test_advance_picks_earliest_in_progress_for_determinism(
    ...
    wiki, slug = plan_with_steps
    set_step(wiki_root=wiki, slug=slug, step_id="step-2", status="in_progress")
    set_step(wiki_root=wiki, slug=slug, step_id="step-3", status="in_progress")
    update = advance(wiki_root=wiki, slug=slug)
    assert update is not None
    assert update.step_id == "step-2"  # earliest by document order
...
def test_auto_close_when_last_step_done(plan_with_steps: tuple[Path, str]) -> None:
    ...
    wiki, slug = plan_with_steps
    set_step(wiki_root=wiki, slug=slug, step_id="step-1", status=StepStatus.DONE)
    set_step(wiki_root=wiki, slug=slug, step_id="step-2", status=StepStatus.DONE)
    ...
    set_step(
        wiki_root=wiki, slug=slug, step_id="step-3", status=StepStatus.DONE, now=now
    )
    fm = parse_frontmatter(plan_path(wiki, slug).read_text())
...
def test_auto_close_does_not_overwrite_terminal_status(
    ...
    set_step(wiki_root=wiki, slug=slug, step_id="step-1", status=StepStatus.DONE)
    set_step(wiki_root=wiki, slug=slug, step_id="step-2", status=StepStatus.DONE)
    set_step(wiki_root=wiki, slug=slug, step_id="step-3", status=StepStatus.DONE)
...
def test_concurrent_set_step_serializes(plan_with_steps: tuple[Path, str]) -> None:
    ...
    wiki, slug = plan_with_steps
    args = [
        (str(wiki), slug, "step-1", "done"),
        (str(wiki), slug, "step-2", "in_progress"),
        (str(wiki), slug, "step-3", "blocked"),
        (str(wiki), slug, "step-1", "in_progress"),  # last writer wins for s1
    ]
    ...
    assert "step-1" in final_status
    assert final_status["step-2"] == "in_progress"
    assert final_status["step-3"] == "blocked"
