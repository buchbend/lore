...
Lore is glue between hosts (Claude Code, Cursor, VSCode Copilot, future
IDEs and agent frameworks). Each host names its tools differently:
Claude Code's ``Edit`` is Cursor's ``edit_file`` is Copilot's
``applyEdit``. Downstream consumers (noteworthy cascade, Curator B
surfaces, the knowledge graph) should not care which host a turn came
from — they reason over :data:`lore_core.types.ToolCategory`.
...
Adding a new host: extend the mapping here, not downstream. That
guarantees feature extractors and surface generators pick the new
host up for free.
"""
...
# Host-specific tool names → canonical category. Keyed by the raw
# ``ToolCall.name`` as emitted by each adapter. Case-sensitive to stay
# close to the wire format each host uses.
...
def classify_tool_name(host: str, name: str) -> "ToolCategory":
    """Map a host-specific tool name to its canonical category.
    ...
    extend the appropriate per-host map here.
    """
    if not name:
        return "other"
    if host == "claude-code":
